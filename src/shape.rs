    ...
    fn try_from(range: AxisRange) -> Result<Self, Self::Error> {
        match range {
            ...
            AxisRange::In(range, step) => {
                let start = range.start.try_into().map_err(Error::Index)?;
                let stop = range.end.try_into().map_err(Error::Index)?;
...
impl Range {
    ...
    pub fn invert_coord(&self, coord: Coord) -> Result<Coord, Error> {
        if coord.len() < self.len() {
...
impl Shape {
    ...
    pub fn validate(&self) -> Result<(), Error> {
        if self.0.is_empty() {
            return Err(Error::Bounds(format!("invalid shape {:?}", self)));
        }

        let mut size = 1u64;
        for dim in self.0.iter().copied() {
            let dim = if dim == 0 || dim > u32::MAX as u64 {
                Err(Error::Bounds(format!("invalid dimension: {}", dim)))
    ...
    pub fn validate_axes(&self, axes: &[usize]) -> Result<(), Error> {
        match axes.iter().max() {
            Some(max) if *max > self.len() => Err(Error::Bounds(format!(
                "shape {:?} has no axis {}",
                self, max
            ))),
            _ => Ok(()),
    ...
    pub fn validate_range(&self, range: &Range) -> Result<(), Error> {
        if self.contains_range(range) {
            Ok(())
        } else {
            Err(Error::Bounds(format!(
                "shape {:?} does not contain range {:?}",
                self, range
            )))
        }
    }
    ...
    pub fn validate_coord(&self, coord: &[u64]) -> Result<(), Error> {
