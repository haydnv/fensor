...
where
    S: Stream,
{
    pub fn new(source: S, block_shape: Shape) -> Result<Self, Error> {
...
where
    S: Stream<Item = Result<A, Error>>,
    A: NDArrayRead<DType = T>,
    T: CDatatype,
{
    type Item = Result<ArrayBase<Vec<T>>, Error>;
    ...
    fn poll_next(
        ...
        task::Poll::Ready(loop {
            if this.pending.len() > size {
                ...
                let data = ArrayBase::<Vec<T>>::new(shape, data).map_err(Error::from);
                break Some(data);
            } else {
                match ready!(this.source.as_mut().poll_next(cxt)) {
                    Some(Ok(block)) => match block.read(&this.queue) {
                        Ok(buffer) => match buffer.to_slice() {
                            ...
                            Err(cause) => break Some(Err(Error::from(cause))),
                        },
                        Err(cause) => break Some(Err(Error::from(cause))),
                    },
